...
interface InventoryUpdate {
  ...
  levelId?: string // If exists, we update; otherwise create
}
...
class WooToMedusaMigration {
  ...
  async linkProductToShippingProfile(productId: string): Promise<void> {
    try {
      await this.medusaClient.post('/admin/link', {
        add: [
          {
      ...
      // If the link already exists or endpoint structure is different, try alternative approach
      throw error
    }
  }
    ...
    try {
      ...
      for (const product of products) {
        try {
          ...
          // Update product to use the shipping profile
  ...
  async batchUpdateInventory(updates: InventoryUpdate[]): Promise<void> {
    ...
    for (let i = 0; i < updates.length; i += this.BATCH_SIZE) {
      ...
      try {
        ...
        // Separate into create vs update operations
    ...
    try {
      ...
      for (const variant of variants) {
        try {
          ...
          // Set inventory to 0 if weight is missing or zero
          ...
          // Collect inventory updates for batch processing
          ...
          // Only update prices if not inventory-only mode
  ...
  async updateAllProducts(): Promise<void> {
    try {
      ...
      while (page <= totalPages) {
        ...
        for (const wooProduct of products) {
          ...
          try {
            ...
            const inventoryUpdates = await this.updateProductPricesAndInventory(productId, wooVariations)
      ...
      // Batch update all inventory at once
      ...
      while (page <= totalPages) {
        ...
        for (const wooProduct of products) {
          ...
          try {
            ...
            const inventoryUpdates = await this.updateProductPricesAndInventory(productId, wooVariations, true)
            allInventoryUpdates.push(...inventoryUpdates)
